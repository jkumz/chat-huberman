...
def tracker(mock_db_connection):
    ...
    with patch('components.video_processing_tracker.sqlite3.connect', return_value=mock_db_connection):
...
def test_create_table(tracker, mock_db_connection):
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='video_processing_status'")
...
def test_start_processing(tracker, mock_db_connection):
    ...
    cursor = mock_db_connection.cursor()
    cursor.execute("SELECT status, start_time FROM video_processing_status WHERE video_id = ?", (video_id,))
...
def test_complete_processing(tracker, mock_db_connection):
    ...
    cursor = mock_db_connection.cursor()
    cursor.execute("SELECT status, end_time FROM video_processing_status WHERE video_id = ?", (video_id,))
    result = cursor.fetchone()
