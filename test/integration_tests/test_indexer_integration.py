...
def test_process_and_index_chunks_flow(indexer, mock_external_services):
    ...
    chunks = ["This is chunk 1This is chunk 1This is chunk 1", "This is chunk 2This is chunk 2This is chunk 2"]
    video_id = "test_video"

    indexer.process_and_index_chunks(url, chunks, video_id)

    # Verify that external services were called correctly
    mock_external_services['ytdl'].return_value.__enter__.return_value.extract_info.assert_called_once_with(url, download=False)
...
def test_batch_processing(indexer, mock_external_services):
    url = "https://www.youtube.com/watch?v=test_video"
    chunks = ["ChunkChunkChunkChunkChunkChunk"] * 150  # Create 150 chunks to test batch processing
    video_id = "test_video"
