...
def test_scrape_and_preprocess_handles_youtube_loader_exception(scraper, mock_youtube_loader):
...
def test_scrape_and_preprocess_with_empty_transcript(scraper, mock_youtube_loader):
    ...
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content="")]
    result = scraper.scrape_and_preprocess(count=1)
...
def test_scrape_and_preprocess_with_short_transcript(scraper, mock_youtube_loader):
    ...
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content="Short content")]
    result = scraper.scrape_and_preprocess(count=1)
    assert len(result) == 1, "Expected one item for short transcript"
    assert len(result[0]['chunks']) == 1, f"Expected one chunk for short transcript, got {len(result[0]['chunks'])}"
    assert len(result[0]['chunks'][0]) == len("Short content"), "Chunk should contain entire short transcript"

# Edge case: Test behavior when YouTube returns a transcript exactly at the chunk size limit

def test_scrape_and_preprocess_with_exact_chunk_size_transcript(scraper, mock_youtube_loader):
    # Create a transcript that's exactly 99999 characters long
    exact_content = "a" * 99999
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content=exact_content)]
    result = scraper.scrape_and_preprocess(count=1)
    
    assert len(result) == 1, "Expected one item for exact chunk size transcript"
    assert len(result[0]['chunks']) == 1, f"Expected one chunk for exact chunk size transcript, got {len(result[0]['chunks'])}"
    assert len(result[0]['chunks'][0]) == 99999, f"Expected chunk size of 99999, got {len(result[0]['chunks'][0])}"
...
def test_scrape_and_preprocess_with_multiple_chunks(scraper, mock_youtube_loader):
    ...
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content=multi_chunk_content)]
    result = scraper.scrape_and_preprocess(count=1)
    
    assert len(result) == 1, "Expected one item for multi-chunk transcript"
    assert len(result[0]['chunks']) == 2, f"Expected two chunks for multi-chunk transcript, got {len(result[0]['chunks'])}"
    assert len(result[0]['chunks'][0]) == 99999, f"Expected first chunk size of 99999, got {len(result[0]['chunks'][0])}"
    assert len(result[0]['chunks'][1]) == 50001, f"Expected second chunk size of 50001, got {len(result[0]['chunks'][1])}"
...
def test_scrape_and_preprocess_with_small_transcript(scraper, mock_youtube_loader):
    ...
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content=small_content)]
    result = scraper.scrape_and_preprocess(count=1)
    
    assert len(result) == 1, "Expected one item for small transcript"
    assert len(result[0]['chunks']) == 1, f"Expected one chunk for small transcript, got {len(result[0]['chunks'])}"
    assert len(result[0]['chunks'][0]) == 50000, f"Expected chunk size of 50000, got {len(result[0]['chunks'][0])}"
...
def test_scrape_and_preprocess_with_large_transcript(scraper, mock_youtube_loader):
    ...
    mock_youtube_loader.from_youtube_url.return_value.load.return_value = [Mock(page_content=large_content)]
    result = scraper.scrape_and_preprocess(count=1)
    
    assert len(result) == 1, "Expected one item for large transcript"
    assert len(result[0]['chunks']) == 3, f"Expected three chunks for large transcript, got {len(result[0]['chunks'])}"
    assert len(result[0]['chunks'][0]) == 99999, f"Expected first chunk size of 99999, got {len(result[0]['chunks'][0])}"
    assert len(result[0]['chunks'][1]) == 99999, f"Expected second chunk size of 99999, got {len(result[0]['chunks'][1])}"
    assert len(result[0]['chunks'][2]) == 50002, f"Expected third chunk size of 50002, got {len(result[0]['chunks'][2])}"
